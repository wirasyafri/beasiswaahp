...
<div>
	<a href="<?=base_url(akses().'/beasiswa/add');?>" class="btn btn-primary btn-flat">Tambah Beasiswa</a>
</div>
<p>&nbsp;</p>
	...
	<tbody>
		...
		if(!empty($data))
		{
			...
			<tr>
				...
				<td><?=$row->kuota;?></td>
				<td>
					<a href="<?=base_url(akses().'/beasiswa/edit');?>?id=<?=$id;?>" class="btn btn-xs btn-info">Edit</a>
					<a onclick="return confirm('Yakin ingin menghapus beasiswa ini?');" href="<?=base_url(akses().'/beasiswa/delete');?>?id=<?=$id;?>" class="btn btn-xs btn-danger">Delete</a>
				</td>
			</tr>
			<?php
			}
